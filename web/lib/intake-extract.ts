...
import { ensureSourceScopeAmbiguity, type IntakeExtraction, intakeExtractionSchema } from "@/lib/intake";
...
/**
 * Message shown when a clone has no model credentials. The published cases and
 * golden demo still work fully offline, so this only guards the bring-your-own
 * live path (a pasted or dictated note).
 */
...
export async function extractIntake(source: string, locale: IntakeLocale): Promise<IntakeExtraction> {
  ...
  if (!object) throw generationError instanceof Error ? generationError : new Error("Intake extraction failed.");

  return ensureSourceScopeAmbiguity(source, object, locale);
}
